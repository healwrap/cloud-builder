...
const useResponsiveDimension = (
	...
	useEffect(() => {
		...
		const calc = () => {
			...
			let v: string | number | undefined = config[key] as
				| string
				| number
				| undefined;
			...
			else if (w <= 768 && typedConfig["tablet" + k])
				v = typedConfig["tablet" + k];
			else if (w <= 1024 && typedConfig["desktop" + k])
				v = typedConfig["desktop" + k];
			setVal(v);
		};
	...
	return responsive
		? val
		: (config as Record<keyof GradualBlurProps, string | number | undefined>)[
				key
		  ];
};
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
	...
	const { hoverIntensity, animated, onAnimationComplete, duration } =
		config as Partial<GradualBlurProps>;
	useEffect(() => {
